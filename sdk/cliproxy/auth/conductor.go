...
const (
	...
	weeklyQuotaBackoffThreshold    = 11                    // BackoffLevel >= 11 triggers weekly cap detection
	weeklyQuotaRetryAfterThreshold = time.Hour             // Retry-After > 1h triggers weekly cap detection
	defaultWeeklyQuotaCooldown     = 7 * 24 * time.Hour    // 7-day cooldown for weekly cap
)
...
type Manager struct {
	...
	// inflight tracks the number of active requests per auth ID to avoid
	// concurrent requests hitting the same credential under high load.
	inflightMu sync.Mutex
	inflight   map[string]int32

	// usageCount tracks the total number of successful requests per auth ID.
	// When maxUsagePerAuth > 0 and the count reaches the limit, the auth is
	// automatically removed from the pool.
	usageMu        sync.Mutex
	usageCount     map[string]int32
	maxUsagePerAuth int32
...
func NewManager(store Store, selector Selector, hook Hook) *Manager {
	...
	manager := &Manager{
		...
		usageCount:       make(map[string]int32),
		maxUsagePerAuth:  0,
	...
	manager.scheduler.inflightFn = manager.authInflight
	return manager
}

// acquireAuth increments the inflight counter for the given auth ID.
func (m *Manager) acquireAuth(authID string) {
	m.inflightMu.Lock()
	m.inflight[authID]++
	m.inflightMu.Unlock()
}

// releaseAuth decrements the inflight counter for the given auth ID.
func (m *Manager) releaseAuth(authID string) {
	m.inflightMu.Lock()
	if m.inflight[authID] > 0 {
		m.inflight[authID]--
	}
	if m.inflight[authID] == 0 {
...
// authInflight returns the current inflight count for the given auth ID.
func (m *Manager) authInflight(authID string) int32 {
	m.inflightMu.Lock()
	n := m.inflight[authID]
	m.inflightMu.Unlock()
	return n
...
// SetMaxUsagePerAuth configures the maximum number of successful requests
// allowed per auth before it is automatically removed. 0 disables the limit.
func (m *Manager) SetMaxUsagePerAuth(n int32) {
	m.usageMu.Lock()
	m.maxUsagePerAuth = n
	m.usageMu.Unlock()
}

// reserveUsage atomically pre-increments the usage counter for an auth
// before execution begins. Returns false if the auth has already reached
// its usage limit and should not be used. This prevents concurrent
// requests from exceeding the configured max-usage-per-auth.
func (m *Manager) reserveUsage(authID string) bool {
	m.usageMu.Lock()
	defer m.usageMu.Unlock()
	if m.maxUsagePerAuth <= 0 {
		return true
	}
	if m.usageCount[authID] >= m.maxUsagePerAuth {
		return false
	}
	m.usageCount[authID]++
	return true
...
// unreserveUsage decrements the usage counter for an auth when a request
// fails. Only successful requests should consume usage quota.
func (m *Manager) unreserveUsage(authID string) {
	m.usageMu.Lock()
	defer m.usageMu.Unlock()
	if m.maxUsagePerAuth <= 0 {
		return
	}
	if m.usageCount[authID] > 0 {
		m.usageCount[authID]--
	}
	if m.usageCount[authID] == 0 {
		delete(m.usageCount, authID)
	}
}

// checkUsageLimitReached checks whether the auth has reached its usage
// limit and should be removed. Called after a successful request completes
// (the counter was already incremented by reserveUsage).
func (m *Manager) checkUsageLimitReached(authID string) bool {
	m.usageMu.Lock()
	defer m.usageMu.Unlock()
	if m.maxUsagePerAuth <= 0 {
		return false
...
// cleanupUsage removes the usage counter entry for an auth that has been removed.
func (m *Manager) cleanupUsage(authID string) {
	m.usageMu.Lock()
	delete(m.usageCount, authID)
	m.usageMu.Unlock()
}
...
// RemoveAuth permanently removes a credential from memory, scheduler, registry, store, and disk.
// When the store supports backfill (max-auth-files mode), a replacement is loaded from the standby pool.
func (m *Manager) RemoveAuth(ctx context.Context, authID string) error {
	m.mu.Lock()
	auth, ok := m.auths[authID]
	if !ok || auth == nil {
		m.mu.Unlock()
	...
	m.cleanupUsage(authID)
	if m.scheduler != nil {
		m.scheduler.removeAuth(authID)
	}
	registry.GetGlobalRegistry().UnregisterClient(authID)
	if m.store != nil {
		_ = m.store.Delete(ctx, authID)
	}
	if filePath != "" {
		_ = os.Remove(filePath)
	}

	// Backfill: load a replacement from the standby pool if available.
	if bf, ok := m.store.(Backfiller); ok {
		if newAuths, errBf := bf.Backfill(ctx, 1); errBf == nil {
			for _, newAuth := range newAuths {
				if newAuth == nil || newAuth.ID == "" {
					continue
				}
				newAuth.EnsureIndex()
				m.mu.Lock()
				if _, exists := m.auths[newAuth.ID]; !exists {
					m.auths[newAuth.ID] = newAuth.Clone()
				}
				m.mu.Unlock()
				if m.scheduler != nil {
					m.scheduler.upsertAuth(newAuth)
				}
				m.hook.OnAuthRegistered(ctx, newAuth.Clone())
			}
		}
	}
...
func (m *Manager) executeCountMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int) (cliproxyexecutor.Response, error) {
	...
	for {
		...
		for _, upstreamModel := range models {
			...
			// Count-token requests should not consume usage quota.
			m.unreserveUsage(auth.ID)
			m.hook.OnResult(execCtx, result)
			m.releaseAuth(auth.ID)
			return resp, nil
...
func (m *Manager) executeStreamMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int) (*cliproxyexecutor.StreamResult, error) {
	...
	for {
		...
		// Wrap the stream channel to release the auth when the stream completes.
		streamResult = m.wrapStreamRelease(streamResult, auth.ID)
		return streamResult, nil
	}
}

// wrapStreamRelease wraps a StreamResult's Chunks channel so that releaseAuth
// is called exactly once when the upstream channel is fully drained or closed.
func (m *Manager) wrapStreamRelease(sr *cliproxyexecutor.StreamResult, authID string) *cliproxyexecutor.StreamResult {
	if sr == nil || sr.Chunks == nil {
		m.releaseAuth(authID)
		return sr
	}
	wrapped := make(chan cliproxyexecutor.StreamChunk)
	go func() {
		defer close(wrapped)
		defer m.releaseAuth(authID)
...
func (m *Manager) MarkResult(ctx context.Context, result Result) {
		...
		if result.Success {
			...
			if result.Model != "" {
				...
				case 404:
					...
					shouldSuspendModel = true
				case 429:
					// keyword match → delete credential immediately
					if shouldDeleteByKeyword(result.Error) {
						shouldDeleteAuth = true
					}
					var next time.Time
					backoffLevel := state.Quota.BackoffLevel
					isWeeklyCap := false
					if result.RetryAfter != nil && *result.RetryAfter > weeklyQuotaRetryAfterThreshold {
						isWeeklyCap = true
					}
					if backoffLevel >= weeklyQuotaBackoffThreshold {
						isWeeklyCap = true
					}
					if isWeeklyCap {
						next = now.Add(defaultWeeklyQuotaCooldown)
						// Do not increment BackoffLevel for weekly cap
	...
	if shouldDeleteAuth {
		reason := "401"
		if result.Error != nil {
			for _, kw := range _cpaDeleteKeywords {
				if strings.Contains(result.Error.Message, kw) {
					reason = kw
					break
				}
			}
		}
		log.Infof("auto-removing auth %s due to %s", result.AuthID, reason)
		m.cleanupUsage(result.AuthID)
		_ = m.RemoveAuth(ctx, result.AuthID)
		m.hook.OnResult(ctx, result)
		return
	}

	// Usage quota was pre-reserved in pickNextMixed via reserveUsage.
	// On success, check if the limit has been reached and remove the auth.
	// On failure, the caller (execute*MixedOnce) is responsible for calling
	// unreserveUsage after all model retries are exhausted.
	if result.Success && m.checkUsageLimitReached(result.AuthID) {
		log.Infof("auto-removing auth %s due to usage limit reached (%d)", result.AuthID, m.maxUsagePerAuth)
		_ = m.RemoveAuth(ctx, result.AuthID)
		m.hook.OnResult(ctx, result)
		return
	}
...
func resetModelState(state *ModelState, now time.Time) {
	...
	if state.Quota.BackoffLevel > 0 {
		state.Quota.BackoffLevel = state.Quota.BackoffLevel / 2
	}
	state.Quota.Exceeded = false
	state.Quota.Reason = ""
	state.Quota.NextRecoverAt = time.Time{}
	state.Quota.WeeklyCap = false
	state.UpdatedAt = now
}
...
func clearAuthStateOnSuccess(auth *Auth, now time.Time) {
	...
	if auth.Quota.BackoffLevel > 0 {
		auth.Quota.BackoffLevel = auth.Quota.BackoffLevel / 2
	}
	auth.Quota.WeeklyCap = false
	auth.LastError = nil
	auth.NextRetryAfter = time.Time{}
	auth.UpdatedAt = now
}
...
// _cpaDeleteKeywords lists error message keywords that trigger automatic
// credential deletion. When an error message contains any of these strings,
// the auth is considered permanently invalid and will be removed.
var _cpaDeleteKeywords = []string{
	"额度获取失败：401",
	`"status":401`,
	`"status": 401`,
...
// shouldDeleteByKeyword returns true if the error message contains any of the
// keywords in _cpaDeleteKeywords that indicate the credential is permanently
// invalid and should be auto-removed.
func shouldDeleteByKeyword(err *Error) bool {
	if err == nil {
		return false
	}
	msg := err.Message
...
// isUsageLimitReached returns true if the error message contains the
// "usage_limit_reached" marker returned by Codex when a free-tier account
// has exhausted its quota.
func isUsageLimitReached(err *Error) bool {
	if err == nil {
		return false
	}
	return strings.Contains(err.Message, "usage_limit_reached")
}
...
func applyAuthFailureState(auth *Auth, resultErr *Error, retryAfter *time.Duration, now time.Time) {
	...
	case 429:
		...
		var next time.Time
		isWeeklyCap := false
		if retryAfter != nil && *retryAfter > weeklyQuotaRetryAfterThreshold {
			isWeeklyCap = true
		}
		if auth.Quota.BackoffLevel >= weeklyQuotaBackoffThreshold {
			isWeeklyCap = true
		}
		if isWeeklyCap {
			next = now.Add(defaultWeeklyQuotaCooldown)
			auth.Quota.WeeklyCap = true
			// Do not increment BackoffLevel for weekly cap
...
func (m *Manager) pickNextMixedLegacy(ctx context.Context, providers []string, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, string, error) {
	...
	// Pre-reserve usage quota before execution to prevent concurrent requests
	// from exceeding the configured max-usage-per-auth limit.
	if !m.reserveUsage(authCopy.ID) {
		return nil, nil, "", &Error{Code: "auth_not_found", Message: "auth usage limit reached"}
	}
