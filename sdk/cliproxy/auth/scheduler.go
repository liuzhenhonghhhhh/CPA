...
type authScheduler struct {
	...
	// inflightFn returns the number of active requests for a given auth ID.
	// When set, round-robin selection prefers auths with zero inflight requests.
	inflightFn func(string) int32
}
...
func (s *authScheduler) pickSingle(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, error) {
	...
	if picked := shard.pickReadyLocked(preferWebsocket, s.strategy, predicate, s.inflightFn); picked != nil {
		return picked, nil
	}
...
func (s *authScheduler) pickMixed(ctx context.Context, providers []string, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, string, error) {
	...
	if pinnedAuthID != "" {
		...
		if picked := shard.pickReadyLocked(false, s.strategy, predicate, s.inflightFn); picked != nil {
	...
	if s.strategy == schedulerStrategyFillFirst {
		for providerIndex, providerKey := range normalized {
			...
			picked := shard.pickReadyAtPriorityLocked(false, bestPriority, s.strategy, predicate, s.inflightFn)
			if picked != nil {
				return picked, providerKey, nil
			}
		}
	...
	for offset := 0; offset < len(normalized); offset++ {
		...
		picked := shard.pickReadyAtPriorityLocked(false, bestPriority, schedulerStrategyRoundRobin, predicate, s.inflightFn)
		if picked == nil {
			continue
		}
...
func (m *modelScheduler) pickReadyLocked(preferWebsocket bool, strategy schedulerStrategy, predicate func(*scheduledAuth) bool, inflightFn func(string) int32) *Auth {
	if m == nil {
		return nil
	}
	...
	return m.pickReadyAtPriorityLocked(preferWebsocket, priorityReady, strategy, predicate, inflightFn)
}
...
// The caller must ensure expired entries are already promoted when needed.
func (m *modelScheduler) pickReadyAtPriorityLocked(preferWebsocket bool, priority int, strategy schedulerStrategy, predicate func(*scheduledAuth) bool, inflightFn func(string) int32) *Auth {
	if m == nil {
		return nil
	}
	bucket := m.readyByPriority[priority]
	...
	if strategy == schedulerStrategyFillFirst {
		...
		picked = view.pickRoundRobin(predicate, inflightFn)
	}
...
// When inflightFn is provided, it first attempts to find an auth with zero inflight requests
// (idle-first pass). If all matching auths are busy, it falls back to normal round-robin.
func (v *readyView) pickRoundRobin(predicate func(*scheduledAuth) bool, inflightFn func(string) int32) *scheduledAuth {
	if len(v.parentOrder) > 1 && len(v.children) > 0 {
		return v.pickGroupedRoundRobin(predicate, inflightFn)
	}
	if len(v.flat) == 0 {
		return nil
	}

	// Pass 1: prefer auths with zero inflight requests.
	if inflightFn != nil {
		start := 0
		if len(v.flat) > 0 {
		...
		for offset := 0; offset < len(v.flat); offset++ {
			...
			if entry.auth != nil && inflightFn(entry.auth.ID) > 0 {
				continue
			}
			v.cursor = index + 1
			return entry
	...
	// Pass 2 (fallback): normal round-robin ignoring inflight state.
	start := 0
	if len(v.flat) > 0 {
		start = v.cursor % len(v.flat)
	}
...
// When inflightFn is provided, idle auths are preferred in the first pass.
func (v *readyView) pickGroupedRoundRobin(predicate func(*scheduledAuth) bool, inflightFn func(string) int32) *scheduledAuth {
	// Pass 1: prefer auths with zero inflight requests.
	if inflightFn != nil {
		if picked := v.pickGroupedRoundRobinInner(predicate, inflightFn); picked != nil {
			return picked
		}
	}
	// Pass 2 (fallback): normal selection ignoring inflight state.
	return v.pickGroupedRoundRobinInner(predicate, nil)
}

// pickGroupedRoundRobinInner is the core grouped round-robin logic.
// When inflightFilter is non-nil, entries with inflight > 0 are skipped.
func (v *readyView) pickGroupedRoundRobinInner(predicate func(*scheduledAuth) bool, inflightFilter func(string) int32) *scheduledAuth {
	start := 0
	...
	for offset := 0; offset < len(v.parentOrder); offset++ {
		...
		for itemOffset := 0; itemOffset < len(child.items); itemOffset++ {
			...
			if inflightFilter != nil && entry.auth != nil && inflightFilter(entry.auth.ID) > 0 {
				continue
			}
