...
func (s *Service) applyRetryConfig(cfg *config.Config) {
	...
	if cfg.MaxUsagePerAuth > 0 {
		s.coreManager.SetMaxUsagePerAuth(int32(cfg.MaxUsagePerAuth))
	}
}
...
func (s *Service) Run(ctx context.Context) error {
	...
	// In pool mode (max-auth-files > 0), FileTokenStore.List() loads auth entries
	// directly into coreManager bypassing the watcher's synthesizer pipeline.
	// Those entries lack model registration because the NoopHook on coreManager
	// does not trigger registerModelsForAuth. Walk all loaded auths now and
	// register their models so they become available for request routing.
	if s.coreManager != nil {
		auths := s.coreManager.List()
		registered := 0
		for _, item := range auths {
			if item == nil || item.ID == "" || item.Disabled {
				continue
			}
			auth, ok := s.coreManager.GetByID(item.ID)
			if !ok || auth == nil {
		...
		if registered > 0 {
			log.Infof("registered models for %d existing auth(s) loaded from store", registered)
		}
	}
